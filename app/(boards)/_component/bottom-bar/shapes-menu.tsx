     ...
     <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild className="no-ring">
          <Button size="sm" variant="ghost" className="relative w-6 h-6 p-2" 
        ...
        <DropdownMenuContent className="mt-5 flex flex-col gap-y-1 border-none bg-primary-black py-4 text-white">
          {item.value.map((elem) => (
            <DropdownMenuItem
              ...
              className={`flex h-fit justify-between bg-white text-black gap-10 rounded-none px-5 py-3 focus:border-none ${
                activeElement.value === elem?.value ? "bg-[#4AAF6C] text-white pointer-events-none" : "hover:bg-green-300 "
              }`}
            >
              <div className="group flex items-center gap-2">
                ...
                <p
                  className={`text-sm ${
                    activeElement.value === elem?.value ? "text-white" : "text-black"
                  }`}
                >
                  {elem?.name}
                </p>
